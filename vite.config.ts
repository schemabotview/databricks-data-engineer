...
//
// `base` is `/databricks-data-engineer/` for the production BUILD only (the app deploys under
// graphl.in/databricks-data-engineer/ as a concept app in the GraphL catalog, so built asset URLs must be
...
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/databricks-data-engineer/' : '/',
  plugins: [react()],
  resolve: {
